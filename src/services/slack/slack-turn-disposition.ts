...
import { isStopExplainingTurnSignalKind, isUnexpectedTurnStopMessage } from "./slack-conversation-utils.js";
...
export function planCompletedTurnDisposition(options: {
  ...
  if (options.dispatchMessages.length > 0 && options.dispatchMessages.every((message) => isUnexpectedTurnStopMessage(message))) {
    return { kind: "none" };
  }

  const signalKind = options.latestSession.lastTurnSignalTurnId === options.turnId ? options.latestSession.lastTurnSignalKind : undefined;
