...
export async function buildSlackThreadBaseInstructions(options: BuildSlackThreadBaseInstructionsOptions): Promise<string> {
  ...
  return renderTemplate(template, {
    ...
    channel_id: options.channelId,
    thread_ts: options.rootThreadTs,
    post_message_command: `curl -sS -X POST ${options.brokerHttpBaseUrl}/slack/post-message -H 'content-type: application/json' -d '${messagePayload}'`,
    post_state_final_command: `curl -sS -X POST ${options.brokerHttpBaseUrl}/slack/post-state -H 'content-type: application/json' -d '${finalStatePayload}'`,
    post_state_wait_command: `curl -sS -X POST ${options.brokerHttpBaseUrl}/slack/post-state -H 'content-type: application/json' -d '${waitStatePayload}'`,
    post_state_block_command: `curl -sS -X POST ${options.brokerHttpBaseUrl}/slack/post-state -H 'content-type: application/json' -d '${blockStatePayload}'`,
    post_file_command: `curl -sS -X POST ${options.brokerHttpBaseUrl}/slack/post-file -H 'content-type: application/json' -d '${filePayload}'`,
    ...
    thread_history_command: `curl -sS '${options.brokerHttpBaseUrl}/slack/thread-history?channel_id=${encodeURIComponent(options.channelId)}&thread_ts=${encodeURIComponent(options.rootThreadTs)}&before_ts=older-message-ts&limit=20&format=text'`,
