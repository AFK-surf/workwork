...
import type { JsonLike, PersistedAgentTraceEvent, PersistedAgentTurnUsage, SlackSessionRecord } from "../../types.js";
...
export class AgentTraceRecorder {
  ...
  constructor(options: { readonly sessions: SessionManager }) {
  ...
  #resolveSession(event: AgentRuntimeEvent): SlackSessionRecord | undefined {
    ...
    return this.#sessions.listSessions().find((session) => Boolean(agentSessionId && session.agentSessionId === agentSessionId) || Boolean(turnId && session.activeTurnId === turnId));
  }
  ...
  async #recordUsage(session: SlackSessionRecord, event: Extract<AgentRuntimeEvent, { type: "agent.usage.updated" }>): Promise<void> {
    if (!event.turnId) {
      return;
    }
    const now = new Date().toISOString();
...
function inputTraceRole(source: unknown): "system" | "user" {
  ...
  return ["background_job", "background_job_event", "runtime_reminder", "unexpected_turn_stop", "admin_session_reset", "broker_recovery", "recovered_thread_batch"].includes(value) ? "system" : "user";
}
