...
| Session start/resume       | App mention starts or resumes a thread session.                                                                                      | Group @bot starts or resumes a Feishu topic session; replies in that topic use the same lifecycle as Slack thread replies.                                        | Parity                   | `test/e2e-broker*.test.ts`, `test/feishu-codex-bridge.test.ts`, `test/feishu-fixture-replay.test.ts`                         |
| Active follow-up           | Thread replies continue the active Slack session.                                                                                    | Feishu topic replies continue the same session; rootless non-@ group follow-up continues the active Feishu session only in `all` mode.                            | Parity when approved     | `test/feishu-codex-bridge.test.ts`, real smoke `im:message.group_msg` evidence                                               |
