...
import { DEFAULT_HEALTH_CHECK_TIMEOUT_MS, ReleaseDeploymentService } from "../src/services/deploy/release-deployment-service.js";
...
describe("ReleaseDeploymentService", () => {
  ...
  it("deploys split npm package releases without building source on the live host", async () => {
    ...
    const currentWorkerAfterDeploy = path.resolve(path.dirname(fixture.currentWorkerReleasePath), await fs.readlink(fixture.currentWorkerReleasePath));
    expect(currentWorkerAfterDeploy).toBe(packageRootFor(fixture.releasesRoot, "worker", "@agent-session-broker/worker", "0.2.0"));
    ...
    const currentAfterSecondDeploy = path.resolve(path.dirname(fixture.currentWorkerReleasePath), await fs.readlink(fixture.currentWorkerReleasePath));
    const previousAfterSecondDeploy = path.resolve(path.dirname(fixture.previousWorkerReleasePath), await fs.readlink(fixture.previousWorkerReleasePath));
    expect(currentAfterSecondDeploy).toBe(packageRootFor(fixture.releasesRoot, "worker", "@agent-session-broker/worker", "0.1.0"));
    expect(previousAfterSecondDeploy).toBe(packageRootFor(fixture.releasesRoot, "worker", "@agent-session-broker/worker", "0.2.0"));

    await service.rollback({ target: "worker" });
    const currentAfterRollback = path.resolve(path.dirname(fixture.currentWorkerReleasePath), await fs.readlink(fixture.currentWorkerReleasePath));
    const previousAfterRollback = path.resolve(path.dirname(fixture.previousWorkerReleasePath), await fs.readlink(fixture.previousWorkerReleasePath));
    expect(currentAfterRollback).toBe(packageRootFor(fixture.releasesRoot, "worker", "@agent-session-broker/worker", "0.2.0"));
    expect(previousAfterRollback).toBe(packageRootFor(fixture.releasesRoot, "worker", "@agent-session-broker/worker", "0.1.0"));
  ...
  it("deploys admin releases without restarting worker", async () => {
    ...
    expect(status.targets.admin.currentRelease.targetPath).toBe(packageRootFor(fixture.releasesRoot, "admin", "@agent-session-broker/admin", "0.5.0"));
  ...
  it("schedules the admin launchd restart from the installed admin package current symlink", async () => {
    ...
    expect(status.targets.admin.currentRelease.targetPath).toBe(packageRootFor(fixture.releasesRoot, "admin", "@agent-session-broker/admin", "0.4.0"));
    expect(scheduledAdminRestarts).toHaveLength(1);
  ...
  it("keeps legacy single-package deploy options out of the service contract", async () => {
    const serviceSource = await fs.readFile(new URL("../src/services/deploy/release-deployment-service.ts", import.meta.url), "utf8");
...
function createPackageExec(options: { readonly commands: string[]; readonly versions: readonly string[]; readonly setLaunchdLoaded: (loaded: boolean) => void; readonly isLaunchdLoaded: () => boolean }) {
