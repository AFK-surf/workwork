...
describe("real Feishu smoke evidence evaluator", () => {
  ...
  it("loads preflight posture through the package-script --env-file path without leaking secret values", async () => {
    ...
    const result = await runFeishuSmokeCli(["--", "--preflight", `--env-file=${envFile}`, "--json"]);
  ...
  it("rejects missing CLI option values before swallowing the next flag", async () => {
    const result = await runFeishuSmokeCli(["--base-url", "--json"]);
    const output = `${result.stdout}\n${result.stderr}`;
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "preflight.feishu_api_base_china",
          status: "fail",
          evidence: expect.arrayContaining(["FEISHU_API_BASE_URL=https://open.feishu.cn/open-apis (query/hash omitted)", "normalized_domain=invalid"]),
        }),
      ]),
    );
    ...
    const report = evaluateFeishuSmokeStatus(
      {
        ...
        state: {
          ...
          recentBrokerLogs: status.state.recentBrokerLogs.filter((record) => {
            ...
            return message !== "chat.message.deduped" && message !== "chat.platform.degraded" && message !== "chat.outbound.failed";
          }),
        },
      },
      {
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("missing behavior evidence: deduped, degraded, failed")]),
        }),
      ]),
    );
  });
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "observability.behavior_coverage",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("missing behavior evidence: degraded")]),
        }),
      ]),
    );
  });
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          id: "observability.behavior_coverage",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("missing behavior evidence: accepted, deduped")]),
        }),
      ]),
    );
  });
    ...
    const report = evaluateFeishuSmokeStatus(
      {
        ...
        state: {
          ...status.state,
          recentBrokerLogs: [...status.state.recentBrokerLogs, ...acceptedIgnoredTwins],
        },
      },
      {
        FEISHU_DOMAIN: "feishu",
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "observability.behavior_coverage",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("missing behavior evidence: failed")]),
        }),
      ]),
    );
  });
    ...
    const report = evaluateFeishuSmokeStatus(
      {
        ...
        state: {
          ...status.state,
          recentBrokerLogs: [
            ...status.state.recentBrokerLogs.filter((record) => (record as { readonly message?: string }).message !== "chat.outbound.failed"),
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("chat.handler.failed: missing handler=message|interactive")]),
        }),
        expect.objectContaining({
          id: "observability.behavior_coverage",
          status: "fail",
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "observability.required_log_fields",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("chat.handler.failed: missing errorClass")]),
        }),
        expect.objectContaining({
          id: "observability.behavior_coverage",
          status: "fail",
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          id: "observability.behavior_coverage",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("missing behavior evidence: deduped")]),
        }),
      ]),
    );
  });
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "observability.behavior_coverage",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("missing behavior evidence: recovered")]),
        }),
      ]),
    );
  });
    ...
    const report = evaluateFeishuSmokeStatus(
      {
        ...
        state: {
          ...status.state,
          recentBrokerLogs: status.state.recentBrokerLogs.filter((record) => (record as { readonly meta?: { readonly platform?: string } }).meta?.platform !== "slack"),
        },
      },
      {
        FEISHU_DOMAIN: "feishu",
  ...
  it("requires Slack reply evidence to occur after the matching Slack event", () => {
    const status = passingStatus();
    const slackLogs = status.state.recentBrokerLogs.filter((record) => (record as { readonly meta?: { readonly platform?: string } }).meta?.platform === "slack");
    ...
    const report = evaluateFeishuSmokeStatus(
      {
        ...
        state: {
          ...status.state,
          recentBrokerLogs: [...slackLogs.toReversed(), ...nonSlackLogs],
        },
      },
      {
        FEISHU_DOMAIN: "feishu",
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          status: "fail",
          evidence: expect.arrayContaining(["platforms.slack.state=ready", "connection.mode=socket_mode", "connection.connected=false", "connection.lastConnectedAt=missing"]),
        }),
      ]),
    );
  });
    ...
    const report = evaluateFeishuSmokeStatus(
      {
        ...
        state: {
          ...status.state,
          recentBrokerLogs: [slackPlatformLog("chat.platform.ready"), ...status.state.recentBrokerLogs],
        },
      },
      {
        FEISHU_DOMAIN: "feishu",
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "slack.socket_mode_ready",
          status: "pass",
          evidence: expect.arrayContaining([expect.stringContaining("chat.platform.ready slack socket_mode")]),
        }),
      ]),
    );
  });
    ...
    const report = evaluateFeishuSmokeStatus(
      {
        ...
        state: {
          ...status.state,
          recentBrokerLogs: status.state.recentBrokerLogs.filter((record) => (record as { readonly message?: string }).message !== "chat.platform.ready"),
        },
      },
      {
        FEISHU_DOMAIN: "feishu",
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          status: "fail",
          evidence: expect.arrayContaining(["platforms.feishu.state=ready", "connection.mode=unknown", "connection.lastConnectedAt=missing"]),
        }),
      ]),
    );
  });
    ...
    const report = evaluateFeishuSmokeStatus(
      {
        ...
        state: {
          ...status.state,
          recentBrokerLogs: [log("chat.platform.ready", { platform: "feishu" }), ...status.state.recentBrokerLogs],
        },
      },
      {
        FEISHU_DOMAIN: "feishu",
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "feishu.long_connection_ready",
          status: "pass",
          evidence: expect.arrayContaining(["connection.mode=long_connection", "connection.connected=true", "connection.lastConnectedAt=2026-03-19T00:00:02.000Z"]),
        }),
      ]),
    );
  });
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          status: "fail",
          evidence: expect.arrayContaining(["slack.connection.mode=unknown", "feishu.connection.mode=unknown", "feishu.permissions=missing", expect.stringContaining("platforms.feishu.permissions.im:message.group_msg")]),
        }),
      ]),
    );
  });
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "admin.platform_health_contract",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("platforms.slack.connection.lastConnectedAt"), expect.stringContaining("platforms.feishu.connection.connected=true")]),
        }),
      ]),
    );
  });
    ...
    const report = evaluateFeishuSmokeStatus(
      {
        ...
        state: {
          ...status.state,
          recentBrokerLogs: [...transitionLogs, ...nonTransitionLogs],
        },
      },
      {
        FEISHU_DOMAIN: "feishu",
        ...
        state: {
          ...
          recentBrokerLogs: [
            ...status.state.recentBrokerLogs.map((record) => {
              const logRecord = record as { readonly message?: string; readonly meta?: Record<string, unknown> };
              if (logRecord.message !== "chat.message.accepted" && logRecord.message !== "chat.session.created") {
                return record;
              }
              if (logRecord.message === "chat.message.accepted" && logRecord.meta?.route !== "bot_mention") {
                return record;
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          id: "observability.behavior_coverage",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("missing behavior evidence: accepted, deduped, failed, recovered")]),
        }),
      ]),
    );
  });
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          id: "observability.behavior_coverage",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("missing behavior evidence: ignored")]),
        }),
      ]),
    );
  });
  ...
  it("does not report persisted-session private ignored logs as private-ignore evidence", () => {
    ...
    const behaviorCoverage = report.checks.find((check) => check.id === "observability.behavior_coverage");
    const evidence = [...(ignored?.evidence ?? []), ...(behaviorCoverage?.evidence ?? [])].join("\n");
    ...
    const report = evaluateFeishuSmokeStatus(
      {
        ...
        state: {
          ...status.state,
          recentBrokerLogs: status.state.recentBrokerLogs.filter((record) => (record as { readonly meta?: Record<string, unknown> }).meta?.ignoredReason !== "ignored_self"),
        },
      },
      {
        FEISHU_DOMAIN: "feishu",
    ...
    const report = evaluateFeishuSmokeStatus(
      {
        ...
        state: {
          ...
          recentBrokerLogs: status.state.recentBrokerLogs.map((record) => {
            const logRecord = record as { readonly message?: string; readonly meta?: Record<string, unknown> };
            if (logRecord.message !== "chat.message.accepted" || logRecord.meta?.route !== "group_message" || logRecord.meta?.messageId !== "om") {
              return record;
            }
  ...
  it("requires non-@ follow-up transition evidence to occur after the accepted message", () => {
    ...
    const followupTransitionLogs = status.state.recentBrokerLogs.filter((record) => {
      const logRecord = record as { readonly message?: string; readonly meta?: Record<string, unknown> };
      return logRecord.message === "chat.turn.steered" && logRecord.meta?.platform === "feishu" && logRecord.meta?.source !== "history_recovery";
    });
    ...
    const report = evaluateFeishuSmokeStatus(
      {
        ...
        state: {
          ...status.state,
          recentBrokerLogs: [...followupTransitionLogs, ...nonTransitionLogs],
        },
      },
      {
        FEISHU_DOMAIN: "feishu",
    ...
    const report = evaluateFeishuSmokeStatus(
      {
        ...
        state: {
          ...status.state,
          sessions: [...status.state.sessions, otherSession],
          recentBrokerLogs: status.state.recentBrokerLogs.map((record) => {
            const logRecord = record as { readonly message?: string; readonly meta?: Record<string, unknown> };
            if (logRecord.message === "chat.message.accepted" && logRecord.meta?.route === "group_message" && logRecord.meta?.msgType === "text" && logRecord.meta?.messageId !== "om_duplicate" && logRecord.meta?.messageId !== "om_stop") {
              return {
                ...logRecord,
                meta: {
                  ...logRecord.meta,
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          id: "observability.required_log_fields",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("chat.turn.stopped: missing active turnId")]),
        }),
      ]),
    );
  });
  ...
  it("requires -stop evidence to occur after the accepted stop transition", () => {
    const status = passingStatus();
    const stoppedLogs = status.state.recentBrokerLogs.filter((record) => (record as { readonly message?: string }).message === "chat.turn.stopped");
    ...
    const report = evaluateFeishuSmokeStatus(
      {
        ...
        state: {
          ...status.state,
          recentBrokerLogs: [...stoppedLogs, ...nonStoppedLogs],
        },
      },
      {
        FEISHU_DOMAIN: "feishu",
    ...
    const report = evaluateFeishuSmokeStatus(
      {
        ...
        state: {
          ...status.state,
          recentBrokerLogs: status.state.recentBrokerLogs.filter((record) => (record as { readonly message?: string }).message !== "chat.turn.completed"),
        },
      },
      {
        FEISHU_DOMAIN: "feishu",
  ...
  it("requires Feishu turn completion evidence to occur after the same-session text reply", () => {
    const status = passingStatus();
    const completionLogs = status.state.recentBrokerLogs.filter((record) => (record as { readonly message?: string }).message === "chat.turn.completed");
    const nonCompletionLogs = status.state.recentBrokerLogs.filter((record) => (record as { readonly message?: string }).message !== "chat.turn.completed");
    const report = evaluateFeishuSmokeStatus(
      {
        ...status,
        state: {
          ...status.state,
          recentBrokerLogs: [...completionLogs, ...nonCompletionLogs],
        },
      },
      {
        FEISHU_DOMAIN: "feishu",
    ...
    const report = evaluateFeishuSmokeStatus(
      {
        ...
        state: {
          ...
          recentBrokerLogs: status.state.recentBrokerLogs.filter((record) => {
            const logRecord = record as { readonly message?: string; readonly meta?: Record<string, unknown> };
            return !((logRecord.message === "chat.turn.steered" || logRecord.message === "chat.turn.started") && logRecord.meta?.source === "history_recovery");
          }),
        },
      },
      {
    ...
    const report = evaluateFeishuSmokeStatus(
      {
        ...
        state: {
          ...status.state,
          recentBrokerLogs: status.state.recentBrokerLogs.filter((record) => (record as { readonly message?: string }).message !== "chat.coauthor.confirmed"),
        },
      },
      {
        FEISHU_DOMAIN: "feishu",
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          id: "observability.required_log_fields",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("chat.card.callback.received: missing conversationId, rootMessageId")]),
        }),
      ]),
    );
  });
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "feishu.card_callback",
          status: "fail",
          evidence: expect.arrayContaining(["requiredSession=group_at outboundCardSessionCount=1"]),
        }),
      ]),
    );
  });
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "feishu.card_callback",
          status: "fail",
          evidence: expect.arrayContaining(["requiredSession=group_at outboundCardSessionCount=0"]),
        }),
      ]),
    );
  });
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "feishu.card_callback",
          status: "fail",
          evidence: expect.arrayContaining(["requiredSession=group_at outboundCardSessionCount=1", "outboundCardMessageCount=1"]),
        }),
      ]),
    );
  });
    ...
    const report = evaluateFeishuSmokeStatus(
      {
        ...
        state: {
          ...status.state,
          recentBrokerLogs: [...status.state.recentBrokerLogs.filter((record) => (record as { readonly message?: string }).message === "chat.card.callback.received"), ...status.state.recentBrokerLogs.filter((record) => (record as { readonly message?: string }).message !== "chat.card.callback.received")],
        },
      },
      {
        FEISHU_DOMAIN: "feishu",
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          id: "observability.required_log_fields",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("chat.card.callback.received: missing candidateRevision when co-author action")]),
        }),
      ]),
    );
  });
    ...
    const report = evaluateFeishuSmokeStatus(
      {
        ...
        state: {
          ...status.state,
          recentBrokerLogs: [...status.state.recentBrokerLogs.filter((record) => (record as { readonly message?: string }).message === "chat.coauthor.confirmed"), ...status.state.recentBrokerLogs.filter((record) => (record as { readonly message?: string }).message !== "chat.coauthor.confirmed")],
        },
      },
      {
        FEISHU_DOMAIN: "feishu",
  ...
  it("requires real setup label evidence when checking final smoke readiness", () => {
    const missing = evaluateFeishuSmokeStatus(
      passingStatus(),
      {
        FEISHU_DOMAIN: "feishu",
      },
    ...
    const complete = evaluateFeishuSmokeStatus(
      passingStatus(),
      {
        FEISHU_DOMAIN: "feishu",
      },
    ...
    expect(complete.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "setup.console_labels_recorded",
          status: "pass",
          evidence: expect.arrayContaining(["im:message.group_msg=approved", "send_message=configured", "card.action.trigger=enabled", "resource_transfer=configured"]),
        }),
      ]),
    );
  });
  ...
  it("validates Feishu setup label evidence shape", () => {
    expect(evaluateFeishuSetupEvidence(passingSetupEvidence())).toEqual(
      expect.objectContaining({
        id: "setup.console_labels_recorded",
        status: "pass",
        evidence: expect.arrayContaining(["setup evidence contains no raw secrets, tokens, user emails, or raw bot IDs", "send_message.apiName=im:message:send_as_bot", "card.action.trigger.eventName=card.action.trigger", "resource_transfer.scopeName=获取与上传图片或文件资源"]),
      }),
    );
    ...
    expect(
      evaluateFeishuSetupEvidence({
        ...
        notes: ["raw bot open id ou_realbot123456"],
      }),
    ).toEqual(
      expect.objectContaining({
        id: "setup.console_labels_recorded",
        status: "fail",
        evidence: expect.arrayContaining([expect.stringContaining("setupEvidence.appSecret")]),
    ...
    expect(
      evaluateFeishuSetupEvidence({
        ...passingSetupEvidence(),
        notes: ["tenant label copied from console", "raw callback actor ou_realbot123456 should not be stored"],
      }),
    ).toEqual(
      expect.objectContaining({
        id: "setup.console_labels_recorded",
        status: "fail",
        evidence: expect.arrayContaining([expect.stringContaining("setupEvidence.notes[1]")]),
      }),
    );
    ...
    expect(
      ...
      expect.objectContaining({
        id: "setup.console_labels_recorded",
        status: "fail",
        evidence: expect.arrayContaining(["target=lark", "im:message.group_msg.apiName=missing", "im:message.group_msg=pending", "approvalEvidence=missing", expect.stringContaining("missing labels:")]),
      }),
    );
    ...
    expect(
      ...
      expect.objectContaining({
        id: "setup.console_labels_recorded",
        status: "fail",
        evidence: expect.arrayContaining(["send_message.apiName=missing", "card.action.trigger.eventName=missing", "resource_transfer.scopeName=missing", expect.stringContaining("missing permission posture:")]),
      }),
    );
    ...
    expect(
      ...
      expect.objectContaining({
        id: "setup.console_labels_recorded",
        status: "fail",
        evidence: expect.arrayContaining(["im:message.group_msg.apiName=missing", "approvalEvidence=missing", "send_message.apiName=missing", "card.action.trigger.eventName=missing", "resource_transfer.scopeName=missing"]),
      }),
    );
  });

  it("rejects the setup evidence example until real tenant labels replace placeholders", async () => {
    const example = JSON.parse(await fs.readFile(path.join(process.cwd(), "docs", "feishu-setup-evidence.example.json"), "utf8")) as unknown;
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "feishu.all_message_verified",
          status: "fail",
          evidence: expect.arrayContaining(["permission.im:message.group_msg=missing"]),
        }),
        expect.objectContaining({
          id: "feishu.group_at_created_session",
          status: "fail",
    ...
    const report = evaluateFeishuSmokeStatus(
      {
        ...
        state: {
          ...
          recentBrokerLogs: status.state.recentBrokerLogs.filter((record) => {
            const logRecord = record as { readonly message?: string; readonly meta?: Record<string, unknown> };
            return !(logRecord.message === "chat.turn.steered" && logRecord.meta?.source !== "history_recovery" && logRecord.meta?.messageId === "om");
          }),
        },
      },
      {
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "feishu.all_message_verified",
          status: "fail",
          evidence: expect.arrayContaining(["groupMessageMode=all", "allMessageDeliveryVerified=true", "sameSessionNonAtFollowup=false", "permission.im:message.group_msg=verified"]),
        }),
        expect.objectContaining({
          id: "feishu.non_at_followup",
          status: "fail",
  ...
  it("returns structured smoke evidence when the admin status endpoint is unavailable", () => {
    ...
    expect(report).toEqual({
      ...
      checks: [
        {
          ...
          required: true,
          status: "fail",
          evidence: ["base_url=http://127.0.0.1:3000/admin", "http_status=404", "response_payload=present"],
      ...
      nextActions: ["admin.status_available: Start a broker build that exposes /admin/api/status, pass the correct --base-url/--admin-token, then rerun the Feishu smoke checker."],
    });
  ...
  it("writes explicit unavailable admin status evidence when the live status fetch fails", async () => {
    ...
    expect(parsedStatus.adminStatus).toEqual({
      ...
      evidence: ["base_url=http://127.0.0.1:3000/admin", "http_status=503", "response_payload=empty"],
      nextAction: "Start a broker build that exposes /admin/api/status, pass the correct --base-url/--admin-token, then rerun the Feishu smoke checker.",
    });
        ...
        checks: [
          {
            ...
            required: true,
            status: "fail",
            evidence: ["Bearer feishu-human-evidence-secret", "FEISHU_HUMAN_BODY"],
            nextAction: "xoxb-feishu-human-next-action-secret",
          },
        ],
        nextActions: ["xapp-feishu-human-next-action-secret"],
      },
      {
  ...
  it("prints sanitized bundle write notices without leaking output directories", () => {
    const evidenceNotice = renderFeishuSmokeBundleNotice("evidence", path.join("/tmp", "FEISHU_OUTPUT_PATH_SECRET", "operator@example.com", "feishu-smoke-summary.md"));
    const preflightNotice = renderFeishuSmokeBundleNotice("preflight", path.join("/tmp", "FEISHU_OUTPUT_PATH_SECRET", "feishu-preflight-summary.md"));
  ...
  it("redacts unsafe values from early CLI error output", () => {
    const fileError = formatFeishuSmokeCliError(new Error("ENOENT: no such file or directory, open '/tmp/FEISHU_CLI_PATH_SECRET/operator@example.com/setup.json'"));
    const argumentError = formatFeishuSmokeCliError(new Error("Unknown argument: --token=xoxb-feishu-cli-secret"));
    const unquotedPathError = formatFeishuSmokeCliError(new Error("Unknown argument: --status-file=/tmp/FEISHU_CLI_UNQUOTED_PATH_SECRET/operator@example.com/status.json"));
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "feishu.rich_card_resource",
          status: "fail",
          evidence: expect.arrayContaining(["requiredSession=group_at groupAtSessionCount=1", "missing msgType: image"]),
        }),
      ]),
    );
  });
    ...
    const report = evaluateFeishuSmokeStatus(
      {
        ...
        state: {
          ...
          recentBrokerLogs: status.state.recentBrokerLogs.filter((record) => {
            const logRecord = record as { readonly message?: string; readonly meta?: Record<string, unknown> };
            return logRecord.message !== "chat.outbound.posted" || logRecord.meta?.platform !== "feishu" || logRecord.meta?.format !== "card";
          }),
        },
      },
      {
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          status: "fail",
          evidence: expect.arrayContaining(["requiredSession=group_at groupAtSessionCount=1", "missing outbound format: card"]),
        }),
      ]),
    );
  });
    ...
    const report = evaluateFeishuSmokeStatus(
      {
        ...
        state: {
          ...
          recentBrokerLogs: status.state.recentBrokerLogs.map((record) => {
            const logRecord = record as { readonly message?: string; readonly meta?: Record<string, unknown> };
            if (logRecord.message !== "chat.outbound.posted" || logRecord.meta?.platform !== "feishu" || logRecord.meta?.format !== "file") {
              return record;
            }
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "feishu.outbound_rich_card_file",
          status: "fail",
          evidence: expect.arrayContaining(["requiredSession=group_at groupAtSessionCount=1", "missing outbound format: file"]),
        }),
      ]),
    );
  });
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          id: "observability.required_log_fields",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("chat.outbound.posted: missing fileId when format=file|image")]),
        }),
      ]),
    );
  });
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "observability.required_log_fields",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("chat.outbound.posted: missing sessionKey")]),
        }),
      ]),
    );
  });
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "observability.required_log_fields",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("chat.session.resumed: missing turnId when active")]),
        }),
      ]),
    );
  });
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "observability.required_log_fields",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("chat.outbound.posted: missing messageId or fileId")]),
        }),
      ]),
    );
  });
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "observability.required_log_fields",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("chat.message.accepted: missing payloadRef")]),
        }),
      ]),
    );
  });
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "observability.required_log_fields",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("chat.message.accepted: missing payloadRef=feishu-message:om_rich")]),
        }),
      ]),
    );
  });
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "observability.required_log_fields",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("chat.card.callback.received: missing payloadRef=feishu-card:evt")]),
        }),
      ]),
    );
  });
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "observability.required_log_fields",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("chat.message.accepted: missing fileId")]),
        }),
      ]),
    );
  });
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "observability.required_log_fields",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("chat.platform.degraded: missing permission")]),
        }),
      ]),
    );
  });
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "observability.required_log_fields",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("chat.history.recovered: missing degradedReason")]),
        }),
      ]),
    );
  });
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "observability.required_log_fields",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("chat.history.recovered: missing recoveredCount")]),
        }),
      ]),
    );
  });
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "observability.required_log_fields",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("chat.attachment.download_failed: missing attachmentId, kind")]),
        }),
      ]),
    );
  });
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("forbidden meta field meta.content")]),
        }),
      ]),
    );
  });
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "observability.no_info_warn_body_leaks",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("forbidden meta field meta.attachmentSummaries[0].rawPayload")]),
        }),
      ]),
    );
  });
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "observability.no_info_warn_body_leaks",
          status: "fail",
          evidence: expect.arrayContaining([expect.stringContaining("forbidden meta value meta.diagnostic.actor")]),
        }),
      ]),
    );
  });
  ...
  it("can evaluate a saved admin status evidence file", async () => {
    ...
    const report = await evaluateFeishuSmokeStatusFile(
      statusFile,
      {
        FEISHU_DOMAIN: "feishu",
      },
      ...
      checks: report.checks.map((check, index) =>
        index === 0
          ? {
              ...
              evidence: [...check.evidence, "Bearer feishu-report-evidence-secret"],
              nextAction: "FEISHU_REPORT_NEXT_ACTION_BODY",
            }
          : check,
      ),
      nextActions: [...report.nextActions, "xoxb-feishu-report-next-action-secret"],
    };
  ...
  it("allowlists recent broker log metadata in reusable evidence bundles", async () => {
    ...
    const statusEvidence = await fs.readFile(bundle.statusFile, "utf8");
    const parsedStatus = JSON.parse(statusEvidence);
    const sanitizedLog = parsedStatus.state.recentBrokerLogs.find((record: { meta?: { eventId?: string } }) => record.meta?.eventId === "evt-secret");
  ...
  it("sanitizes setup evidence copied into reusable evidence bundles", async () => {
    ...
    const setupEvidence = {
      ...
      notes: ["safe redacted ticket reference", "raw callback actor ou_realbot123456 should not be stored", "Bearer feishu-setup-secret"],
    };
    const report = evaluateFeishuSmokeStatus(
      passingStatus(),
      {
...
function log(message: string, meta: Record<string, unknown>) {
  const defaultMessageId = typeof meta.messageId === "string" && meta.messageId.trim() ? meta.messageId : "om";
  const defaultEventId = typeof meta.eventId === "string" && meta.eventId.trim() ? meta.eventId : "evt";
  const normalizedMeta =
    meta.platform === "feishu"
      ? {
          ...
          batchId: "om",
          payloadRef: message === "chat.card.callback.received" ? `feishu-card:${defaultEventId}` : `feishu-message:${defaultMessageId}`,
...
async function runFeishuSmokeCli(args: readonly string[]): Promise<{
  ...
  const child = spawn(process.execPath, ["--import", "tsx", "test/manual/run-real-feishu-smoke.ts", ...args], {
    cwd: process.cwd(),
    env: {
