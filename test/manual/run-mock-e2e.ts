...
async function main(): Promise<void> {
  ...
  try {
    ...
    const historyResponse = await fetch("http://127.0.0.1:3300/slack/thread-history?channel_id=C123&thread_ts=111.220&before_ts=111.221&limit=1&format=text");
    ...
    await mockSlack.waitForPostedMessage((message) => message.text.includes("查看会话活动时间线"));
    ...
    await mockSlack.waitForPostedMessage((message) => message.channel === "D123" && message.text.includes("查看会话活动时间线"));
